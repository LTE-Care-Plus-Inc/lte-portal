...
import { UserPlus, ShieldCheck, Phone, Mail, ChevronRight, DollarSign, Calendar, Lock } from 'lucide-react';
  ...
  const handleSubmit = async (e) => {
    ...
    try {
        ...
        if (data.status === 'success') {
            ...
            setStatus({ message: `Critical Error: ${data.message || 'Check Logs'}`, type: 'error' });
        }
    } catch (err) {
        setStatus({ message: `Network Failure: ${err.message}`, type: 'error' });
    }
  };
      ...
      <div className="w-full max-w-2xl bg-white/5 border border-white/10 rounded-[2.5rem] overflow-hidden backdrop-blur-3xl shadow-2xl animate-in fade-in slide-in-from-bottom-8 duration-700">
        ...
        <div className="p-10 border-b border-white/10 bg-white/[0.02]">
          <div className="flex justify-between items-start">
            <div>
              <h2 className="text-3xl font-black tracking-tighter uppercase mb-1 italic">BT Onboarding</h2>
              <p className="text-white/40 text-[10px] uppercase tracking-[0.3em] font-bold">Automation Module v2.5</p>
            </div>
            <div className="bg-blue-500/10 border border-blue-500/20 px-3 py-1 rounded-full text-blue-400">
               <div className="flex items-center gap-2">
                 <div className="w-1.5 h-1.5 bg-current rounded-full animate-pulse"></div>
                 <span className="text-[9px] font-black uppercase tracking-widest text-blue-400">System Ready</span>
               </div>
            </div>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="p-10 space-y-8">
          ...
          {/* Pay Rate & End Date Section */}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {/* LOCKED PAY RATE FIELD */}
            <InputGroup label="Hourly Pay Rate (Locked)" icon={<Lock size={12} className="text-emerald-500/50" />}>
              <div className="w-full bg-emerald-500/5 border border-emerald-500/20 rounded-2xl px-5 py-4 text-sm font-black flex items-center justify-between group">
                <span className="text-emerald-400/90 tracking-tighter text-lg flex items-center gap-1">
                    <DollarSign size={16} /> 22.00
                </span>
                <span className="text-[8px] uppercase tracking-widest text-emerald-500/40">Standard BT</span>
              </div>
            </InputGroup>
        ...
        {status.message && (
          <div className="p-10 bg-white/[0.02] border-t border-white/10 animate-in slide-in-from-bottom-4 duration-500">
             ...
             `}>
                ...
                <div>
                   <p className="text-[9px] font-black uppercase tracking-[0.2em] opacity-40 mb-1">System Feedback</p>
